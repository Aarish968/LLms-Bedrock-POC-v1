    ...
    df = pd.read_sql(
            f"""with prep_separators as (
                select UID, ID, CAMCECID, ACCOUNTNAME, SFCFA,SERVICE_LEVEL, EXCLUSION_NOTES,
                       SFC_AGREEMENT_TYPE, CREATED_BY, CREATE_DTM, UPDATED_BY, UPDATE_DTM, IS_ACTIVE, CONTRACT_NUMBER,FILE_UPLOAD_STATUS, FILE_UPLOAD_DTM
                        ,replace(replace(replace(replace(replace(replace( h.SMARTACCOUNTID, ' ', ' '),' ', ' '),' ',' '), ' ', ','), ',,', ','), ',,', ',') as SMARTACCOUNTID
                        ,replace(replace(replace(replace(replace(replace(h.MCEENGAGEMENTID, ' ', ' '),' ', ' '),' ',' '), ' ', ','), ',,', ','), ',,', ',') as MCEENGAGEMENTID
                from CPS_BIA_BR.DATA_CANVAS_ENGAGEMENT_HDR_V h
                where length(SMARTACCOUNTID) > 1
            ), smart as (
                select distinct m.ENGAGEMENT_number, try_to_date(m.DATE_BUCKET, 'YYYY-MM-DD_HH24_MI_SS') as new_date_bucket, try_to_date(m.DATE_BUCKET, 'AUTO') as old_date_bucket,
                DATE_BUCKET, FILE_LOCATION
                from CPS_DSCI_ARCHIVE.MCE_ENGAGEMENT_TRACKING_META m
                where coalesce(new_date_bucket, old_date_bucket) > to_date('2021-10-27', 'AUTO')
                  and coalesce(m.FILE_LOCATION,'') != ''
                  and m.IS_PREPPED is null  -- fill in correct flag
                  and m.SMART_ACCOUNT_ID::varchar(500) in
                      (
                          SELECT distinct value as ACAT_ACCOUNT_ID
                          FROM prep_separators,
                              LATERAL STRTOK_SPLIT_TO_TABLE(prep_separators.SMARTACCOUNTID, ',')
                      )
           ...
           , engage as (
                        ...
                        where coalesce(new_date_bucket, old_date_bucket) > to_date('2021-10-27', 'AUTO')
                            and coalesce(m.FILE_LOCATION,'') != ''
                            and m.IS_PREPPED is null  -- fill in correct flag
                        and m.ENGAGEMENT_number::varchar(100) in
                        ( -- requested by a cam
                            SELECT distinct value::varchar(100)
                            FROM prep_separators,
                            LATERAL STRTOK_SPLIT_TO_TABLE(prep_separators.MCEENGAGEMENTID, ',')
                        )
        )
            select *
            from smart
...
def trigger_flow_run(
    ...
    return prefect_client.create_flow_run(
        version_group_id="e1419b9d-3e94-465e-9048-228db49663cf",
        parameters=dict(
