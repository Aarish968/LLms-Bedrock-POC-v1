...
@setup_extra_loggers
def handle_canvas_view_refresh(
    env: TEnv | Env,
    canvas_id: int,
    notification_id: int,
    dc_user_id: int,
    ...
    """Refresh the ThoughtSpot data and Liveboards for a canvas"""
    ...
    notify_block.send_text("Rebuilding the Thoughtspot Data and Liveboards")
    ...
    with ErrorHandler(
        error_message="Failed to refresh TS datasource", notification_block=notify_block
    ):
        canvas_service.refresh_ts_datasource()

    notify_block.mark_successful(message="Rebuilt the ThoughtSpot Data and Liveboards")
