...
real_sql = f"""SELECT d.request_id, customer_id, customer_name,
    ...
    try:
        if not os.path.exists(out_name):
            ...
            if 'offer_ato_suite_description' in df.columns:
                df['offer_ato_suite_description'] = df['offer_ato_suite_description'].replace(
                    ['nan', 'None', '<NA>', np.nan], '')
            ...
            if 'delete_manual_exclusion_flag' in df.columns:
                df['delete_manual_exclusion_flag'] = df['delete_manual_exclusion_flag'].replace(
                    ['nan', 'None', '<NA>', np.nan], '')

            if 'unquotable_instructions_flag' in df.columns:
                df['unquotable_instructions_flag'] = df['unquotable_instructions_flag'].replace(
                    ['nan', 'None', '<NA>', np.nan], '')
            ...
            if 'atc_coverage_start_date' in df.columns:
                df['atc_coverage_start_date'] = df['atc_coverage_start_date'].replace(['nan', 'None', '<NA>', np.nan],
                                                                                      np.nan)

            if 'target_date_terminated' in df.columns:
                df['target_date_terminated'] = df['target_date_terminated'].replace(['nan', 'None', '<NA>', np.nan],
                                                                                    np.nan)

            if 'atc_coverage_end_date' in df.columns:
                df['atc_coverage_end_date'] = df['atc_coverage_end_date'].replace(['nan', 'None', '<NA>', np.nan],
                                                                                  np.nan)
            ...
            if 'target_contract_number' in df.columns:
                df['target_contract_number'] = df['target_contract_number'].replace(['nan', 'None', '<NA>', np.nan],
                                                                                    np.nan)
            ...
            if 'target_install_site_id_flag' in df.columns:
                df['target_install_site_id_flag'] = df['target_install_site_id_flag'].replace(
                    ['nan', 'None', '<NA>', np.nan], '')

            if 'target_install_at_site_use_id' in df.columns:
                df['target_install_at_site_use_id'] = df['target_install_at_site_use_id'].replace(
                    ['nan', 'None', '<NA>', np.nan], -1)

            if 'target_contract_bill_to_id' in df.columns:
                df['target_contract_bill_to_id'] = df['target_contract_bill_to_id'].replace(
                    ['nan', 'None', '<NA>', np.nan], -1)
            ...
            if df.shape[0] > 0:
                ...
                with pq.ParquetWriter(out_name, needed_schema, compression='snappy',
                                      allow_truncated_timestamps=True) as writer:
...
with Flow('proc_acat',
          ...
          executor=LocalDaskExecutor(scheduler="processes", num_workers=35)
          ) as process_acat:
    broken = []
    stats = []
...
def extract_mce_data(local_folder_v2_enrich, lst, dte_run, sf_warehouse, core_query, acat_tbl, notes_tbl):
    ...
    this_fn_data_out_path = file_ops.prep_data_location(os.path.join(local_folder_v2_enrich, dte_run, str(req)),
...
with Flow(
        ...
        run_config=KubernetesRun(memory_request=60000000000),
        # executor=LocalDaskExecutor(scheduler="processes", num_workers=psutil.cpu_count(logical=True)),
        executor=LocalDaskExecutor(scheduler="processes", num_workers=38),
        result=S3Result(bucket="cam-prefect-results")
) as flow:
...
engagements['display_name'] = engagements.apply(
    lambda x: gen_display_name(x['engagement_name_mod'], x['creation_date']), axis=1)
engagements
