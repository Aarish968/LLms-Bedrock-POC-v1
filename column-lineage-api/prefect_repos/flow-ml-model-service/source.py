...
def load_data(eng_id):
    ...
    engine = create_engine(sec.get_sf_pw('prd_cps_dsci_etl_svc', warehouse, schema))

    df= pd.read_sql(get_managed_qry(eng_id), engine)
...
def save_sourced_df(df, eng_id):
    def gen_min_id():
        letters = string.ascii_letters
        random_string = '{}'.format(''.join(random.choice(letters) for i in range(10)))
        return random_string
    today = date.today()
    aws_canvas_out_pth = f"s3://ds-data-store-prod/sourced_data/{eng_id}/{today.strftime('%Y_%m_%d')}"
    file_id = f"{eng_id}_model_{gen_min_id()}"
    aws_path = os.path.join(aws_canvas_out_pth, f"{file_id}.parquet")
    wr.s3.to_parquet(df, path=aws_path, compression='snappy', index=False)
...
def write_message_for_training(eng_id, aws_path,file_id):
    ...
    key = f"""trigger_processing/{file_id}.json"""
...
with Flow("ml-model-source",
    ...
    aws_path,file_id = save_sourced_df(df,eng_id)
