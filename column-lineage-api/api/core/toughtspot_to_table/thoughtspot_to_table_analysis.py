...
# Import existing database connection utilities (same as sp_analyzer.py)
...
class ThoughtSpotCSVAnalysisExtension:
    ...
    def get_sf_connection_engine(self):
        """Get Snowflake connection engine using existing database infrastructure (same as sp_analyzer.py)."""
        try:
    ...
    def get_database_manager(self):
        """Get DatabaseManager instance for query execution (same as sp_analyzer.py)."""
        try:
    ...
    def get_all_views_and_tables(self, sf_env: str = 'prod', include_views: bool = True) -> pd.DataFrame:
        """Get all views and tables from CPS_DSCI schemas using existing database infrastructure."""
        ...
        if db_manager.mock_mode:
            logger.info("Running in mock mode - returning sample table data")
            return pd.DataFrame({
                'TABLE_NAME': ['SAMPLE_TABLE_1', 'SAMPLE_TABLE_2', 'SAMPLE_VIEW_1'],
                'TABLE_SCHEMA': ['CPS_DSCI_API', 'CPS_DSCI_API', 'CPS_DSCI_BR'],
        ...
        logger.info(f"Getting tables/views from CPS_DSCI schemas...")
        logger.info(f"Target schemas: {schemas}, Include views: {include_views}")
        
        try:
            # Use existing database engine
            engine = self.get_sf_connection_engine()
            
            if engine is None:
                logger.warning("No database engine available - returning empty DataFrame")
                return pd.DataFrame(columns=['TABLE_NAME', 'TABLE_SCHEMA', 'TABLE_TYPE'])
            ...
            logger.warning("Returning empty DataFrame due to fetch error")
            return pd.DataFrame(columns=['TABLE_NAME', 'TABLE_SCHEMA', 'TABLE_TYPE'])
